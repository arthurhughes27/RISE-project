...
fig_s1 = metrics_figures1 %>% ggplot(aes(x = p)) +
  geom_histogram(binwidth = 0.03, fill = "lightblue", color = "black", alpha = 0.7, aes(y = ..density..)) +
...
figs7 = ggplot(p_evaluate_long, aes(x = as.factor(variable), y = value, fill = as.factor(variable))) +
  ...
  labs(
    x = "Proportion of invalid surrogates",
    y = "P-value",
    title = expression("Evaluation stage P-values as a function of the proportion of false positives in " * gamma[S] *" - DGF 2")) +
  geom_hline(yintercept = 0.05, linetype = "dashed", colour = "red") + 
  theme_minimal(base_size = 18) +
  theme(plot.title = element_text(hjust = 0.5),
        plot.background = element_rect(fill = 'white', color = 'white'),
