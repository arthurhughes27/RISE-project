...
# Evaluation function
Function which repeatedly simulates data and calculates performance metrics (FPR, PFP, TPR).
```{r}
...
# metrics_fig2 = get(load("./output/simulation/simulation/metrics_figure2.Rdata"))
...
figure3 = grid.arrange(
  ...
  top = textGrob("Empirical Power and Proportion of False Positives as a function of average surrogate strength",
...
figures2 = grid.arrange(
  arrangeGrob(TPR_patchwork_figs2, FDR_patchwork_figs2, legend, 
              ncol = 3, widths = c(1, 1, 0.3)),  # Adjust widths as needed
  top = textGrob("Empirical Power and Proportion of False Positives as a function of surrogate strength and multiple testing correction method",
                 gp = gpar(fontsize = 18))  # Adjust fontsize and fontface
)
...
## Figure S3
PFP and empirical power as a function of inter-predictor correlation for a fixed sample size and surrogate strength
```{r}
alpha <- 0.05 # nominal significance level
beta <- 0.2 # desired power for univariate surrogate test = (1-beta)*100% 
s0_mean <- 0 # invalid surrogate mean value in untreated group
...
# metrics_figs5 = get(load("./output/simulation/simulation/metrics_figures5.Rdata"))
...
# FDR as a function of surrogate strength
FDR_plot_unadjusted = metrics_figs6 %>% ggplot(aes(x = avg_us, y = avg_fdr_unadjusted, color = as.factor(n))) + 
  geom_line() + 
  geom_point() +
  ylab("Proportion of False Positives") +
  xlab("U_S") + 
...
TPR_plot_unadjusted  = metrics_figs6 %>% ggplot(aes(x = avg_us, y = avg_tpr_unadjusted , color = as.factor(n))) + 
  geom_line() + 
  geom_point() +
  ylab("Empirical power") +
  xlab("U_S") + 
...
legend = get_legend(metrics_figs6 %>% ggplot(aes(x = avg_us, y = avg_fdr_unadjusted, color = as.factor(n))) + 
                      geom_line() + 
                      geom_point() +
                      ylab("Proportion of False Positives") +
                      xlab("U_S") + 
...
figures6 = grid.arrange(
  arrangeGrob(TPR_patchwork, FDR_patchwork, legend, 
              ncol = 3, widths = c(1, 1, 0.3)),  # Adjust widths as needed
  top = textGrob("Empirical Power and Proportion of False Positives as a function of average surrogate strength - DGF 2",
                 gp = gpar(fontsize = 20))  # Adjust fontsize and fontface
)
...
figs7 = ggplot(p_evaluate_long, aes(x = as.factor(variable), y = value, fill = as.factor(variable))) +
  ...
  labs(
    x = "Proportion of invalid surrogates",
    y = "P-value",
    title = "Evaluation stage P-values as a function of the proportion of false positives in gamma - DGF 2"
  ) +
  geom_hline(yintercept = 0.05, linetype = "dashed", colour = "red") + 
  theme_minimal(base_size = 18) +
  theme(plot.title = element_text(hjust = 0.5),
...
ggsave(plot = figs7, filename = "figures7.pdf",
       path = "./output/figures/",
       units = "cm",
       width = 40,
       height = 25)
