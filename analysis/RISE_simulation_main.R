...
##--Figure 1 - boxplots of FPR as a function of sample size in the uncorrelated setting--##
...
save(metrics_fig1, file = "./output/simulation_results_final/metrics_figure1.Rdata")
...
ggsave(plot = fig1,
       "./output/figures/main/figure1.pdf", 
       units = "cm", 
       width = 40, 
       height = 25)

##--Figure 2 - violin plots of FPR as a function of correlation--##
...
##--Figure 3--##
## Empirical power and proportion of false positives 
## as a function of average surrogate strength 
...
p_grid <- c(500) # keep number of predictors constant
n_sim = 500
...
save(metrics_fig3, file = "./output/simulation_results_final/metrics_figure3.Rdata")

metrics_fig3 = get(load("./output/simulation_results_final/metrics_figure3.Rdata"))

# FDR as a function of surrogate strength
FDR_plot_unadjusted = metrics_fig3 %>% ggplot(aes(x = avg_us, y = avg_fdr_unadjusted, color = as.factor(n))) + 
  geom_line() + 
  geom_point() +
...
TPR_plot_unadjusted  = metrics_fig3 %>% ggplot(aes(x = avg_us, y = avg_tpr_unadjusted , color = as.factor(n))) + 
  geom_line() + 
  geom_point() +
...
legend = get_legend(metrics_fig3 %>% ggplot(aes(x = avg_us, y = avg_fdr_unadjusted, color = as.factor(n))) + 
                      geom_line() + 
                      geom_point() +
                      ylab("Proportion of False Positives") +
                      xlab("U_S") + 
...
figure3 = grid.arrange(
  ...
  top = textGrob("Empirical Power and Proportion of False Positives as a function of surrogate strength",
                 gp = gpar(fontsize = 20, fontface = "bold"))  # Adjust fontsize and fontface
)

ggsave(plot = figure3,
       "./output/figures/main/figure3.pdf", 
       units = "cm", 
       width = 35, 
       height = 18)
